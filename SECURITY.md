...
`MATRIX_AutoPrompter` keeps provider credentials outside serialized workflows. Ordinary node execution returns saved text and makes no provider request. Only the explicit **Generate Prompt** action may send selected reference images and prompt fields to xAI. Review that request before authorizing provider use.
